    ...
    def train_gan(self):
        ...
        for ticker in stock_tickers:
            ...
            for num_window, (train_df, test_df, window_info) in enumerate(windowed_data):
                ...
                check_interval = self.args.check_interval  # 10 에포크마다 모니터링 수행
                patience = self.args.patience    # 개선이 없으면 얼리스탑을 위한 patience (에포크 단위)
                tolerance = self.args.tolerance   # 손실 개선에 대한 허용 오차
                ks_threshold = self.args.ks_threshold      # KS 통계량 임계치 (예시)
                pvalue_threshold = self.args.pvalue_threshold  # KS p-value 임계치 (예시)
                ...
                for epoch in progress:
                    ...
                    for i, data in enumerate(dataloader, 0):
                        ...
                        lossD_real = torch.mean(torch.log(netD(real) + 1e-8))
                        lossD_fake = torch.mean(torch.log(1. - netD(fake) + 1e-8))
                        lossD = -(lossD_real + lossD_fake)
                        lossD.backward()
                        optD.step()
                        
                        # ---------------------
                        # Generator undate (undate by 5 mini-batch)
                        if i % 5 == 0:
                            netG.zero_grad()
                            lossG = torch.mean(torch.log(1. - netD(netG(noise)) + 1e-8))
                            lossG.backward()
                            optG.step()
                    ...
                    progress.set_description(f'Epoch {epoch} | Loss_D: {avg_lossD:.8f}, Loss_G: {avg_lossG:.8f}')
                    
                    # Monitoring & EarlyStop
                    if (epoch % check_interval == 0) and (epoch >= min_epochs):
                        
                        # extracted noise distribution about Real data & Generated data
                        real_noise = Ticker_processed.flatten()
                        generated_noise = self.extract_learned_noise(num_sequences=self.args.num_noise_samples, seq_length=len(Ticker_log_train), nz=self.args.noise_input_size, generator=netG, device=self.device)
                        
                        # K-S test
                        ks_stat, ks_pvalue = ks_2samp(real_noise, generated_noise)
                        print(f"Epoch {epoch}: KS statistic = {ks_stat:.6f}, p-value = {ks_pvalue:.6f}")
                        
                        # 조건 1: KS test 통계치 기준 만족 시 얼리스탑
                        if ks_stat < ks_threshold and ks_pvalue > pvalue_threshold:
                            print("Early stopping triggered by KS test criteria.")
                            break
                        
                        # 조건 2: generator 손실이 개선되지 않으면 얼리스탑
                        if avg_lossG < best_loss - tolerance:
                            best_loss = avg_lossG
                            no_improve_counter = 0
                        else:
                            no_improve_counter += check_interval
                        if no_improve_counter >= patience:
                            print("Early stopping triggered due to no improvement in generator loss.")
                            break
